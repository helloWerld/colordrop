/**
 * Gemini native image output as primary conversion provider.
 *
 * Model env: see `getGeminiImageConversionModel` in `@/lib/conversion-model-env`
 * (block comment lists image-capable model IDs and doc links).
 */

import { GoogleGenAI } from "@google/genai";
import type { LineartConversionResult } from "@/lib/convert";
import { getGeminiImageConversionModel } from "@/lib/conversion-model-env";
...
export async function runLineartConversion(
  ...
  const result = await runLineartConversionWithMetadata(imageUrl, dimensions);
...
export async function runLineartConversionWithMetadata(
  ...
  const model = getGeminiImageConversionModel();
  const ai = new GoogleGenAI({ apiKey });
  const response = await ai.models.generateContent({
    model,
    contents: [
      {
        text: lineartPromptWithDimensions(
          dimensions.width,
          dimensions.height,
        ),
      },
